...
class film_calibration:
    ...
    def image_acquisition(self):
        ...
        second = [i for i, s in enumerate(files) if "002" in s]
        second_img = files[second]

        third = [i for i, s in enumerate(files) if "003" in s]
        third_img = files[third]

        fourth = [i for i, s in enumerate(files) if "004" in s]
    ...
    def EBT_fit(self, doses, num_fitting_params):
        ...
        #tmpnetOD = np.insert(self.netOD[:],[0:7,0:7],0)
