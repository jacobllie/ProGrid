...
def survival_histogram(dose_map, pooled_dose, pooled_survival, dose, kernel_size,mode = "GRID"):
    ...
    if mode == "Control":
        ...
        peak_survival =pooled_survival[:,:,pooled_dose > d85].flatten()
        ...
        ax[0].set_title(r"Peak Survival $\bar{x}$ = %.5f , s = %.5f" %(np.mean(peak_survival),
        ...
        ax[1].set_title(r"Valley Survival $\bar{x}$ = %.5f , s = %.5f" %(np.mean(valley_survival),
                  np.var(valley_survival)))
        ...
        fig.tight_layout()
        # fig.savefig("C:\\Users\\jacob\\OneDrive\\Documents\\Skole\\Master\\data\\Survival Analysis Data\\2D analysis\\Poisson\\poisson_eval_histogram_{}mm_{}Gy.png".format(kernel_size, dose), pad_inches = 0.5, dpi = 300)
