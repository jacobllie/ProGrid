...
if confidence:
    ...
    var_res_low = np.sqrt(np.sum(fit_low.fun**2)/df_low)**2
    var_res_high = np.sqrt(np.sum(fit_low.fun**2)/df_high)**2 #estimated variance


    param_cov_low = var_res_low * hessian_approx_inv_low
    param_cov_high = var_res_high * hessian_approx_inv_high
...
#pooled profile row error
# std_fit_open_low = np.sqrt(np.sum(dose_fit_error(netOD_open[low_img_open], dOD_low_open, param_var_low, fitting_param_low)**2,axis = 2)/dose_map_open.shape[2])
# std_fit_open_high = np.sqrt(np.sum(dose_fit_error(netOD_open[high_img_open], dOD_high_open, param_var_high, fitting_param_high)**2,axis = 2)/dose_map_open.shape[2])
...
print(std_fit_open_low.shape)
print(std_fit_open_high.shape)

print(np.max(std_fit_open_low), np.max(std_fit_open_high))
...
"""
#pooled profile error

std_fit_open_profile_low = np.sqrt(np.sum(std_fit_open_low**2, axis = 0)/len(low_img_open))
std_fit_open_profile_high = np.sqrt(np.sum(std_fit_open_high**2, axis = 0)/len(high_img_open))

#weighting the standard errors differently
# stderr_open_profile = np.sqrt((len(low_img_open) - 1)*std_fit_open_profile_low**2 + \
#                               (len(high_img_open) - 1)*std_fit_open_profile_high**2)/np.sqrt(len(low_img_open) + len(high_img_open) -2)
#
# print(np.max(stderr_open_profile))

#total fit error
# stderr_open_profile = np.sqrt((std_fit_open_profile_low**2 + std_fit_open_profile_high**2))/2

stderr_open_profile = std_fit_open_profile_low
...
conf_open = t.ppf(0.95, len(low_img_open) - 2) * stderr_open_profile

#We have the mean of the mean, we therefore include both 16 films and number of pixels on the rows.
# print(t.ppf(0.95, dose_map_open.shape[2] + len(low_img_open) + len(high_img_open) - 1))
...
grid_mean_std =  np.std(dose_grid, axis = 0)/np.sqrt(len(low_mean_grid) + len(high_mean_grid))

#this conf grid includes standard deviation from finding the mean dose map
conf_grid = t.ppf(0.95, len(low_img_grid) + len(high_img_grid) - 2) * np.sqrt(stderr_grid_profile**2 + grid_mean_std**2)

mean_dose_grid = np.mean(dose_grid, axis = 0)


d90 = np.max(mean_dose_grid)*0.90  #dividing by 0.8 because OD is opposite to dose
print(d90)

d10 = np.min(mean_dose_grid)*1.1
print(d10)
...
for i in range(len(dose_grid)):
    ...
    peak_idx = dose_grid[i] > d90
    valley_idx = dose_grid[i] < d10
...
print(np.mean(dose_open),np.std(dose_open)/np.sqrt(len(dose_open.flatten())),
      np.mean(peak), np.sqrt((np.std(peak)/np.sqrt(len(peak)))**2 + (np.sqrt(np.sum(peak_fit_std**2))/len(peak))**2), np.mean(valley),
      np.sqrt((np.std(valley)/np.sqrt(len(valley)))**2 + (np.sqrt(np.sum(valley_fit_std**2))/len(valley))**2))
...
plt.legend()
# plt.savefig("C:\\Users\\jacob\\OneDrive\\Documents\\Skole\\Master\\data\\EBT3 dosimetry\\310821\\dose_profile_all_films.png", bbox_inches = "tight", pad_inches = 0.1, dpi = 1200)
plt.show()
