...
AIC_results_peak_dist = np.zeros((4, 2))
AIC_results_peak_area = np.zeros((4, 2))
...
for i in range(AIC_results_peak_area.shape[1]):
    ax[i].set_ylabel("AIC")
    ax[i].set_xlabel("# Explanatory Variables")
    ...
    ax[i].legend()
    ax[i].set_xticks(np.arange(1,5,1,dtype = int))
...
fig.tight_layout()
fig.savefig("C:\\Users\\jacob\\OneDrive\\Documents\\Skole\\Master\\data\\Survival Analysis Data\\2D analysis\\Poisson\\AIC_2.png",dpi = 300, bbox_inches = "tight")
