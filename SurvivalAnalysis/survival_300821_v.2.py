...
for i in range(3,4):
    ...
    if open_ == True:
        print("yes im open")
        survival_open = survival_analysis(folder, time, mode[1], position, kernel_size_p[i], dose_path_open, template_file_open, dose = dose, cropping_limits = cropping_limits)
    ...
    if Grid == True:
        ...
        survival_grid = survival_analysis(folder, time, mode[2], position, kernel_size_p[i], dose_path_grid, template_file_grid, dose = dose, cropping_limits = cropping_limits)
