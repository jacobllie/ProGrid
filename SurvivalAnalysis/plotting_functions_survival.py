...
def survival_viz(colony_map, kernel_size, dose_map_reg, cropping_limits, Stripes):
    ...
    if Stripes == True:
        ...
        tmp_seg_mask = np.asarray(pd.read_csv("C:\\Users\\jacob\\OneDrive\\Documents\\Skole\\Master\\data\\Segmentation Results - 15.11.2021\\20112019\\GRID Dots\\A549-2011-02-gridC-A-SegMask.csv"))
...
def pred_vs_true_SC(obs_SC, pred_SC, dose_axis, plot_title):
    ...
    plt.xlabel("Dose [Gy]")
    plt.ylabel("SC")
    plt.plot(dose_axis, obs_SC,"*", label = "Obs. SC")
    plt.plot(dose_axis, pred_SC, "*", label = "Pred. SC")
